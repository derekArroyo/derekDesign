     ...
     backdrop-blur-sm text-3xl xl:text-xl xl:rounded-full'>
                {
                    navData.map((link, index) => (
                        <Link
                            className={`${link.path === pathName && 'text-white'} relative flex items-center group hover:text-accent transistion-all duration-300 ` }
                            href={link.path}
                            key={index}     
                        >

                            {/* tooltip */}
                            <section className='absolute pr-14 right-0 hidden xl:group-hover:flex'>
